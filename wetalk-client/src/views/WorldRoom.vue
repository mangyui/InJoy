<template>
  <div class="talk-room">
    <van-nav-bar class="litheme" :border="false" fixed :title="'世界频道(' + myTalk.numberP + ')'" left-arrow  @click-left="$router.go(-1)">
          ...
          <div class="list-item" v-for="(item,index) in msgList" :key="index">
            <div class="mess-item" v-if="item.type==1&&item.user.id!=user.id">
              <div class="mu-avatar">
                ...
                <img class="icon-sex" :src="item.user.sex==1?'./icons/male.svg':'./icons/female.svg'">
              </div>
